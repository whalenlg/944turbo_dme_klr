...
# FQS0=pos0(neutral), FQS1=pos1(+4 rich), FQS2=pos2(-4 lean), FQS3=pos3(+8 richest)
...
cd "$SCRIPT_DIR"
bash run_dashboard_parallel.sh "$WORKERS" $FQS_TESTS
echo "  iverilog done."

# ── 2. Verilator ─────────────────────────────────────────────────────────────
echo ""
echo "  [2/3] Running Verilator FQS sweep..."
bash run_dashboard_parallel.sh --verilator "$WORKERS" $FQS_TESTS
echo "  Verilator done."
