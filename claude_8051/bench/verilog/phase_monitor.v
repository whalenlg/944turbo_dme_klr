...
always @(posedge clk) begin : dme_phase_monitor
    ...
    if (!rst) begin
        ...
        if ( i8051_tb.i8051_top.u_cpu.iram[7'h25][5] && !ph_coldenrich_prev)
            $display("[PHASE] t=%0d ms  COLD-START ENRICH begin     (bit2Dh set)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        if (!i8051_tb.i8051_top.u_cpu.iram[7'h25][5] &&  ph_coldenrich_prev)
            $display("[PHASE] t=%0d ms  COLD-START ENRICH end       (bit2Dh clr)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        ...
        if ( i8051_tb.i8051_top.u_cpu.iram[7'h25][4] && !ph_coldtiming_prev)
            $display("[PHASE] t=%0d ms  COLD-START TIMING begin     (bit2Ch set)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        if (!i8051_tb.i8051_top.u_cpu.iram[7'h25][4] &&  ph_coldtiming_prev)
            $display("[PHASE] t=%0d ms  COLD-START TIMING end       (bit2Ch clr)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        ...
        if ( i8051_tb.i8051_top.u_cpu.iram[7'h24][3] && !ph_o2_prev)
            $display("[PHASE] t=%0d ms  LAMBDA O2 lean               (bit23h set)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        if (!i8051_tb.i8051_top.u_cpu.iram[7'h24][3] &&  ph_o2_prev)
            $display("[PHASE] t=%0d ms  LAMBDA O2 rich               (bit23h clr)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        ...
        if ( i8051_tb.i8051_top.u_cpu.iram[7'h23][5] && !ph_fuelcut_prev)
            $display("[PHASE] t=%0d ms  FUEL CUT begin               (FuelOffCoast set)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        if (!i8051_tb.i8051_top.u_cpu.iram[7'h23][5] &&  ph_fuelcut_prev)
            $display("[PHASE] t=%0d ms  FUEL CUT end                 (FuelOffCoast clr)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        ...
        if ( i8051_tb.i8051_top.u_cpu.iram[7'h20][5] && !ph_isvovf_prev)
            $display("[PHASE] t=%0d ms  ISV OVERFLOW begin           (isv=0x%02X)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000,
                     i8051_tb.i8051_top.u_cpu.iram[7'h7F]);
        if (!i8051_tb.i8051_top.u_cpu.iram[7'h20][5] &&  ph_isvovf_prev)
            $display("[PHASE] t=%0d ms  ISV OVERFLOW end             (isv=0x%02X)",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000,
                     i8051_tb.i8051_top.u_cpu.iram[7'h7F]);
        ...
        if (i8051_tb.i8051_top.u_cpu.iram[7'h2A] == 8'h00 && ph_wdog_prev != 8'h00)
            $display("[PHASE] t=%0d ms  WATCHDOG EXPIRED",
                     i8051_tb.i8051_top.u_cpu.cycle_count / 6000);
        ph_wdog_prev <= i8051_tb.i8051_top.u_cpu.iram[7'h2A];

        // ---- Periodic STATUS: every 500ms DME time ----
        if (i8051_tb.i8051_top.u_cpu.cycle_count >= ph_next_snap) begin
            ...
            ph_next_snap <= ph_next_snap + 64'd3_000_000;
            $fflush();
        end
